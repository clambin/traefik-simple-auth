...
func TestServer(t *testing.T) {
	...
	defer func() { _ = oidcServer.Shutdown() }()
	...
	t.Run("forwardAuth requests with valid cookie are accepted", func(t *testing.T) {
		...
		r := makeForwardAuthRequest(http.MethodGet, "example.com", "/foo")
		r.AddCookie(sessionStore.Cookie(validSession, "example.com"))
		w := httptest.NewRecorder()
		s.ServeHTTP(w, r)
		assert.Equal(t, http.StatusOK, w.Code)
		assert.Equal(t, 1, sessionStore.ActiveUsers()["foo@example.com"])
	})
	...
	t.Run("forwardAuth requests with valid cookie for logout handler are accepted", func(t *testing.T) {
		...
		assert.Zero(t, sessionStore.ActiveUsers()["foo@example.com"])
	})
	t.Run("oauth callback", func(t *testing.T) {
		// queue up a user session.  this gives us the "code" that would have been generated by the oauth flow
		u := mockoidc.MockUser{Email: "foo@example.com", EmailVerified: true}
		session, err := oidcServer.SessionStore.NewSession("oidc profile email", "", &u, "", "")
		require.NoError(t, err)
		// queue up a CSRF state. this gives us the "state" that would have been generated by the oauth redirect
		state := stateStore.Add("https://example.com")

		v := url.Values{}
		v.Set("state", state)
		v.Set("code", session.SessionID)
		r, _ := http.NewRequest(http.MethodGet, "https://traefik"+OAUTHPath+"?"+v.Encode(), nil)
		w := httptest.NewRecorder()
		s.ServeHTTP(w, r)
		assert.Equal(t, http.StatusTemporaryRedirect, w.Code)
